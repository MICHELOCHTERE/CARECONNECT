...
const s = {
 ...
 stepLabel: { fontSize: 10, color: "#4a7a6a", display: "none" },
 ...
 successCard: { background: "#0d1f1a", border: "1px solid #2a4a3e", borderRadius: 12, padding: 12, fontSize: 11, color: "#4ecba0", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" },
...
function RadioGroup({ options, value, onChange }) {
 ...
 <div style={s.pillRow}>
 {options.map((opt) => (
 <button key={opt} style={value === opt ? s.pillActive : s.pillInactive} onClick={() => onChange(opt)}>
 {opt}
 </button>
 ))}
 </div>
...
function CheckboxGroup({ options, values, onChange }) {
 ...
 <div style={s.pillRow}>
 {options.map((opt) => (
 <button key={opt} style={values.includes(opt) ? s.pillActive : s.pillInactive} onClick={() => toggle(opt)}>
 {opt}
 </button>
 ))}
 </div>
...
function Step1({ data, set }) {
 ...
 <input style={s.input} placeholder="Jane" value={data.firstName} onChange={e => set({ ...data, firstName: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Last Name</label>
 <input style={s.input} placeholder="Smith" value={data.lastName} onChange={e => set({ ...data, lastName: e.target.value })} />
 </div>
 </div>
 <div style={s.field}>
 <label style={s.label}>Email Address</label>
 <input style={s.input} type="email" placeholder="jane@example.com" value={data.email} onChange={e => set({ ...data, email: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Phone Number</label>
 <input style={s.input} type="tel" placeholder="07700 900000" value={data.phone} onChange={e => set({ ...data, phone: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Date of Birth</label>
 <input style={s.input} type="date" value={data.dob} onChange={e => set({ ...data, dob: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Postcode</label>
 <input style={s.input} placeholder="SW1A 1AA" value={data.postcode} onChange={e => set({ ...data, postcode: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Do you hold a valid driving licence?</label>
 <RadioGroup options={["Yes", "No"]} value={data.driving} onChange={v => set({ ...data, driving: v })} />
 </div>
 </div>
 );
}
function Step2({ data, set }) {
 ...
 <div style={s.field}>
 <label style={s.label}>Care Settings (select all that apply)</label>
 ...
 <div style={s.field}>
 <label style={s.label}>Client Groups</label>
 ...
 <CheckboxGroup options={["No formal qualifications", "Care Certificate", "NVQ Level 2", "NVQ Level 3", "First Aid", "Manual Handling"]} values={data.quals} onChange={v => set({ ...data, quals: v })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Preferred Hours</label>
 <CheckboxGroup options={["Days", "Evenings", "Nights", "Weekends", "Flexible"]} values={data.hours} onChange={v => set({ ...data, hours: v })} />
 </div>
 </div>
 );
}
function Step3({ data, set }) {
 ...
 <div>
 <div style={s.field}>
 <label style={s.label}>Do you have the right to work in the UK?</label>
 <RadioGroup options={["Yes", "No", "Not sure"]} value={data.rightToWork} onChange={v => set({ ...data, rightToWork: v })} />
 </div>
 ...
 <div style={s.field}>
 <label style={s.label}>Documents you can provide</label>
 ...
 <div style={s.infoBox}>
 <div style={s.infoTitle}> Right to Work Check</div>
 We are legally required to verify your right to work in the UK before you begin employment. You will be asked to provide original documents at your interview.
 </div>
 </div>
 );
}
function Step4({ data, set }) {
 ...
 <div>
 <div style={s.field}>
 <label style={s.label}>Do you have an existing DBS certificate?</label>
 <RadioGroup options={["Yes — Enhanced", "Yes — Basic", "No"]} value={data.hasDbs} onChange={v => set({ ...data, hasDbs: v })} />
 </div>
 {data.hasDbs?.startsWith("Yes") && (
 <>
 <div style={s.field}>
 <label style={s.label}>DBS Issue Date</label>
 <input style={s.input} type="date" value={data.dbsDate} onChange={e => set({ ...data, dbsDate: e.target.value })} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Are you on the DBS Update Service?</label>
 <RadioGroup options={["Yes", "No"]} value={data.updateService} onChange={v => set({ ...data, updateService: v })} />
 </div>
 </>
 )}
 <div style={{ ...s.field }}>
 <label style={s.label}>Have you ever been convicted of a criminal offence?</label>
 <RadioGroup options={["No", "Yes — declared"]} value={data.conviction} onChange={v => set({ ...data, conviction: v })} />
 </div>
 <div style={s.infoBox}>
 <div style={s.infoTitle}> DBS Requirements</div>
 Working in home care requires an Enhanced DBS check. If you do not already hold one, we will arrange this for you.
 </div>
 </div>
 );
}
function Step5({ data, set }) {
 ...
 <input style={s.input} placeholder="John Doe" value={data.refs[i]?.name || ""} onChange={e => updateRef(i, "name", e.target.value)} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Job Title</label>
 <input style={s.input} placeholder="Care Manager" value={data.refs[i]?.title || ""} onChange={e => updateRef(i, "title", e.target.value)} />
 </div>
 </div>
 <div style={s.field}>
 <label style={s.label}>Organisation</label>
 <input style={s.input} placeholder="Sunrise Care Ltd" value={data.refs[i]?.org || ""} onChange={e => updateRef(i, "org", e.target.value)} />
 </div>
 <div style={s.field}>
 <label style={s.label}>Email</label>
 <input style={s.input} type="email" placeholder="john@company.com" value={data.refs[i]?.email || ""} onChange={e => updateRef(i, "email", e.target.value)} />
 </div>
 <div style={s.field}>
 ...
 By submitting, you confirm all information is accurate and you consent to background checks being carried out as part of the recruitment process.
 </div>
 </div>
 );
}
export default function App() {
 ...
 <p style={s.successText}>Thank you, {p1.firstName || "there"}! We've received your application and will be in touch within 2 working days to arrange your next steps.</p>
 <div style={s.successGrid}>
 {["Interview Booked", "DBS Verified", "Start Date Set"].map((s2, i) => (
 <div key={i} style={s.successCard}>{s2}</div>
 ))}
 </div>
 ...
 <button style={s.btnNext} onClick={() => current === steps.length ? setSubmitted(true) : setCurrent(current + 1)}>
 {current === steps.length ? "Submit Application →" : "Continue →"}
 </button>
 </div>
 </div>
 </div>
